 ...
 * */
bool PQ_is_empty(_PQ_QueueTypeDef* queue) { return queue->head == NULL; }
 ...
 * */
void PQ_insert(_PQ_QueueTypeDef* queue, PQ_PriorityTypeDef priority,
			   void* payload) {
	...
	if (queue->head == NULL || queue->head->priority > new_node->priority) {
		...
		while (cursor->next != NULL &&
			   cursor->next->priority <= new_node->priority)
			cursor = cursor->next;
